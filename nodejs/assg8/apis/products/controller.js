...
exports.createProduct = async (req, res) => {
  try {
    ...
    if (!body.name) {
      ...
      let isProductPresent = await productModel.findOne({
        name: req.body.name,
      });
      if (!isProductPresent) {
...
exports.colour = async (req, res) => {
  try {
    ...
    if (!body.colour) {
      ...
      let colourProducts = await productModel.find({
...
exports.topFive = async (req, res) => {
  try {
    ...
    let topFiveProducts = await productModel.aggregate([
      { $sort: { price: -1 } },
      { $limit: 5 },
...
exports.nthHighestProductsList = async (req, res) => {
  try {
    ...
    if (!body.n) {
      ...
      let n = req.body.n;
      let nthProducts = await productModel.aggregate([
        { $group: { _id: "$price", name: { $push: "$name" } } },
        { $sort: { _id: -1 } },
        { $skip: n - 1 },
        { $limit: 1 },
      ]);
...
exports.profilePic = async (req, res) => {
  try {
    ...
    let response = req.files.profilePic;
    console.log(response);
    let copyImage = fs.copyFileSync(
      response.path,
      __dirname + "/../../images/" + response.originalFilename
    );
    res.send(response);
  } catch (err) {
    res.send(err);
